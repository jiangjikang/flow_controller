...
static void preorder(RBTree tree)
{
	if (tree != NULL)
	{
		rt_kprintf("%d:%d ", tree->key,tree->value);
		preorder(tree->left);
		preorder(tree->right);
	}
}
...
int rbtree_search(RBRoot *root, KeyType key,ValueType *value)
{
	Node *node = NULL;
	if (root)
	{
		node = search(root->node, key);
		if (node)
		{
			*value = node->value;
			return 0;
...
int iterative_rbtree_search(RBRoot *root, KeyType key,ValueType *value)
{
	Node *node = NULL;
	if (root)
	{
		node = iterative_search(root->node, key);
		if (node)
		{
			*value = node->value;
			return 0;
...
int rbtree_minimum(RBRoot *root, KeyType *val)
{
	Node *node = NULL;
...
int rbtree_maximum(RBRoot *root, KeyType *val)
{
	Node *node = NULL;
...
*     right 是右孩子。
*/
static Node* create_rbtree_node(KeyType key,ValueType value, Node *parent, Node *left, Node* right)
{
	Node* p;
	static int count = 0;
...
/*
* 新建结点(节点键值为key)，并将其插入到红黑树中,若已存在key值，则更新value
*
* 参数说明：
*     root 红黑树的根
*     key 插入结点的键值
...
*     -1，插入失败
*/
int insert_rbtree(RBRoot *root, KeyType key,ValueType value)
{
	Node *node = NULL;    // 新建结点
	...
	// 如果新建结点失败，则返回。
	if ((node = create_rbtree_node(key, value, NULL, NULL, NULL)) == NULL)
		return -1;
...
static void rbtree_print(RBTree tree, KeyType key, int direction)
{
	if (tree != NULL)
	{
		if (direction == 0)    // tree是根节点
			rt_kprintf("%2d(B) is root\n", tree->key);
		else                // tree是分支节点
			rt_kprintf("%2d(%s) is %2d's %6s child\n", tree->key, rb_is_red(tree) ? "R" : "B", key, direction == 1 ? "right" : "left");
